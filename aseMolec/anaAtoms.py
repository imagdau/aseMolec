...
def extract_molecs(db, fct=1, intra_inter=False, prog=False, forces_key='forces'):
    moldb = []
    for i, at in enumerate(db):
        ...
        for m in np.unique(molID):
            ...
            if 'initial_charges' in at.arrays:
                ...
                molD.append(D)
            if forces_key in at.arrays:
                Fcm = np.sum(mol.arrays[forces_key], axis=0)
                Tcm = np.sum(np.cross(mol.positions-cm, mol.arrays[forces_key], axis=1), axis=0)
